...
def left_join_json(json1_path, json2_path):
    """Left join, Calcula 'Sales', exluimos 'title' y maneja errores."""
    try:
        ...
        for item1 in data1:
            if item2.get('Product') == item1.get('title'):
                try:
                    ...
                    sales = round(sales, 2)  # Redondear a 2 decimales
                    result.append({
                    ...
                    print(f"Error de tipo al calcular Sales para {item2.get('Product')}. Se usará 0.")
                    sales = 0
                    result.append({
                        'SALE_ID': item2.get('SALE_ID'),
                        'Product': item2.get('Product'),
...
def write_results_to_file(results, output_file, total_sales):
    """Escribe resultados y suma de ventas a un archivo txt"""
    results_with_sales = {"results": results, "total_sales": total_sales}  # Sin tiempo
    try:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="left join")
    ...
    parser.add_argument("-o", "--output", help="Nombre del archivo de salida (opcional)", default="SalesResults.txt")
    ...
    if result is not None:
        ...
        write_results_to_file(result, args.output, total_sales)  # No se pasa elapsed_time

        print(f"\nResultados guardados en {args.output}")
        print(f"Tiempo de ejecución: {elapsed_time:.4f} segundos")  # Solo en consola
